...
{
   TFT_DC_D;     // 数据指令选择 引脚输出高电平，代表本次传输 数据
...
{ 
  ...
  LCD_RST_L;                    //RST 置低
	HAL_Delay(10);
	LCD_RST_H;                    //RST 置高  完成对摄像头的复位
  HAL_Delay(10);               	// 屏幕刚完成复位时（包括上电复位），需要等待至少5ms才能发送指令

 	TFT_SEND_CMD(0x36);       // 用于设置LCD的显示方向（如横屏或竖屏）和颜色格式（RGB或BGR）
	                              // 配置成 从上到下、从左到右，RGB像素格式
	TFT_SEND_DATA(0x00);     //清屏
	TFT_SEND_CMD(0x3A);				// 接口像素格式 指令，用于设置使用 12位、16位还是18位色
	TFT_SEND_DATA(0x05);     // 此处配置成 16位 像素格式
  ...
  // 以下电压设置指令，使用厂家默认值
	...
	TFT_SEND_CMD(0xB7);		   // 栅极电压设置指令	
	TFT_SEND_DATA(0x35);    // VGH = 13.26V，VGL = -10.43V

	TFT_SEND_CMD(0xBB);			 // 公共电压设置指令
	TFT_SEND_DATA(0x19);    // VCOM = 1.35V

	TFT_SEND_CMD(0xC0);
	TFT_SEND_DATA(0x2C);

	TFT_SEND_CMD(0xC2);       // VDV 和 VRH 来源设置
	TFT_SEND_DATA(0x01);     // VDV 和 VRH 由用户自由配置

	TFT_SEND_CMD(0xC3);				// VRH电压 设置指令  
	TFT_SEND_DATA(0x12);     // VRH电压 = 4.6+( vcom+vcom offset+vdv)
				
	TFT_SEND_CMD(0xC4);		   // VDV电压 设置指令	
	TFT_SEND_DATA(0x20);     // VDV电压 = 0v

	TFT_SEND_CMD(0xC6); 		// 正常模式的帧率控制指令
	TFT_SEND_DATA(0x0F);   	// 设置屏幕控制器的刷新帧率为60帧    

	TFT_SEND_CMD(0xD0);			// 电源控制指令
	TFT_SEND_DATA(0xA4);     // 无效数据，固定写入0xA4
	TFT_SEND_DATA(0xA1);     // AVDD = 6.8V ，AVDD = -4.8V ，VDS = 2.3V

	TFT_SEND_CMD(0xE0);       // 正极电压伽马值设定
	TFT_SEND_DATA(0xD0);
	TFT_SEND_DATA(0x04);
	TFT_SEND_DATA(0x0D);
	TFT_SEND_DATA(0x11);
	...
	TFT_SEND_CMD(0x21);       // 打开反显，因为面板是常黑型，操作需要反过来
...
void LCD_SetAddress(uint16_t x1,uint16_t y1,uint16_t x2,uint16_t y2)		
{
	...
	TFT_SEND_CMD(0x2c);			//	开始写入显存，即要显示的颜色数据
}
...
void LCD_SetColor(uint32_t Color)
{
	...
	TFT_Ptr.Color = (uint16_t)(Red_Value | Green_Value | Blue_Value);  // 将颜色写入全局LCD参数		
}
...
void LCD_SetBackColor(uint32_t Color)
{
	...
	TFT_Ptr.BkgColor = (uint16_t)(Red_Value | Green_Value | Blue_Value);	// 将颜色写入全局LCD参数			   	
}
...
{
	TFT_Ptr.Direction = direction;    // 写入全局LCD参数

   if( direction == Direction_H )   // 横屏显示
   {
      TFT_SEND_CMD(0x36);    		// 显存访问控制 指令，用于设置访问显存的方式
      TFT_SEND_DATA(0x70);        // 横屏显示
      TFT_Ptr.X_Offset   = 20;             // 设置控制器坐标偏移量
      TFT_Ptr.Y_Offset   = 0;   
      TFT_Ptr.Width      = LCD_Height;		// 重新赋值长、宽
      TFT_Ptr.Height     = LCD_Width;		
   }
   else if( direction == Direction_V )
   {
      TFT_SEND_CMD(0x36);    		// 显存访问控制 指令，用于设置访问显存的方式
      TFT_SEND_DATA(0x00);        // 垂直显示
      TFT_Ptr.X_Offset   = 0;              // 设置控制器坐标偏移量
      TFT_Ptr.Y_Offset   = 20;     
      TFT_Ptr.Width      = LCD_Width;		// 重新赋值长、宽
      TFT_Ptr.Height     = LCD_Height;						
   }
   else if( direction == Direction_H_Flip )
   {
      TFT_SEND_CMD(0x36);   			 // 显存访问控制 指令，用于设置访问显存的方式
      TFT_SEND_DATA(0xA0);         // 横屏显示，并上下翻转，RGB像素格式
      TFT_Ptr.X_Offset   = 20;              // 设置控制器坐标偏移量
      TFT_Ptr.Y_Offset   = 0;      
      TFT_Ptr.Width      = LCD_Height;		 // 重新赋值长、宽
      TFT_Ptr.Height     = LCD_Width;				
   ...
   {
      TFT_SEND_CMD(0x36);    		// 显存访问控制 指令，用于设置访问显存的方式
      TFT_SEND_DATA(0xC0);        // 垂直显示 ，并上下翻转，RGB像素格式
      TFT_Ptr.X_Offset   = 0;              // 设置控制器坐标偏移量
      TFT_Ptr.Y_Offset   = 20;     
      TFT_Ptr.Width      = LCD_Width;		// 重新赋值长、宽
      TFT_Ptr.Height     = LCD_Height;				
...
*	函 数 名:	LCD_Clear
*
*	函数功能:	清屏函数，将LCD清除为 LCD.BkgColor 的颜色
*
...
void LCD_Clear(void)
{
   LCD_SetAddress(0,0,TFT_Ptr.Width-1,TFT_Ptr.Height-1);	// 设置坐标
  ...
  LCD_CSL;  //拉低cs引脚，开始传输 
   ...
   LCD_SPI_Transmit(&LCD_SPI, TFT_Ptr.BkgColor, TFT_Ptr.Width * TFT_Ptr.Height) ;   // 启动传输
   LCD_CSH;  //拉高cs引脚，传输结束
	// 改回8位数据宽度，因为指令和部分数据都是按照8位传输的
	 LCD_SPI.Init.DataSize 	= SPI_DATASIZE_8BIT;    //	8位数据宽度
   HAL_SPI_Init(&LCD_SPI);
...
*					height - 要清除区域的纵向宽度
*
*	函数功能:	局部清屏函数，将指定位置对应的区域清除为 LCD.BkgColor 的颜色
*
...
// 修改为16位数据宽度，写入数据更加效率，不需要拆分	
   ...
   LCD_SPI_Transmit(&LCD_SPI, TFT_Ptr.BkgColor, width*height) ;  // 启动传输
  LCD_CSH;  //拉低NSS引脚，开始传输
// 改回8位数据宽度，因为指令和部分数据都是按照8位传输的
	LCD_SPI.Init.DataSize 	= SPI_DATASIZE_8BIT;    //	8位数据宽度
   HAL_SPI_Init(&LCD_SPI);
   ...
   uint8_t   disChar;		//存储字符的地址
	...
	for(index = 0; index < LCD_AsciiFonts->Sizes; index++)	
	{
		...
		{ 
			if(disChar & 0x01)	
			{		
            LCD_Buff[i] =  TFT_Ptr.Color;			// 当前模值不为0时，使用画笔色绘点
			}
			else		
			{		
            LCD_Buff[i] = TFT_Ptr.BkgColor;		//否则使用背景色绘制点
			}
			disChar >>= 1;
			i++;
         w++;
...
*					3. 可设置对应的背景色，例如使用 LCD_SetBackColor(0x000000) 设置为黑色的背景色
*					4. 使用示例 LCD_DisplayString( 10, 10, "fendou") ，在起始坐标为(10,10)的地方显示字符串"fendou"
*
*****************************************************************************************************************************************/

void LCD_DisplayString( uint16_t x, uint16_t y, char *p) 
{  
	while ((x < TFT_Ptr.Width) && (*p != 0))	//判断显示坐标是否超出显示区域并且字符是否为空字符
	{
...
*					4. 使用示例 LCD_DisplayChinese( 10, 10, "奋") ，在坐标(10,10)显示中文字符"奋"
*
*****************************************************************************************************************************************/
...
*					2.	可设置要显示的颜色，例如使用 LCD_SetColor(0xff0000FF) 设置为蓝色
*					3. 可设置对应的背景色，例如使用 LCD_SetBackColor(0xff000000) 设置为黑色的背景色
*					4. 使用示例 LCD_DisplayChinese( 10, 10, "奋斗STM32") ，在坐标(10,10)显示字符串"奋斗STM32"
*
*******************************************************/
	...
	for(i = 0; i <height; i++)             // 循环按行写入
	{
		for(j = 0; j <(float)width/8; j++)  
		{
			...
			{ 
				if(disChar & 0x01)	
				{		
               LCD_Buff[BuffCount] =  TFT_Ptr.Color;			// 当前模值不为0时，使用画笔色绘点
				}
				else		
				{		
				   LCD_Buff[BuffCount] = TFT_Ptr.BkgColor;		//否则使用背景色绘制点
				}
				disChar >>= 1;     // 模值移位
				Xaddress++;        // 水平坐标自加
				BuffCount++;       // 缓冲区计数       
